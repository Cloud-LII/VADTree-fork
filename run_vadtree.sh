...
if [ "$LLM_MODEL_DIR" != "/path/to/LLM_MODEL_DIR" ]; then
    ...
    # Note: These patterns match the actual output structure generated by the LLaVA-Video-7B-Qwen2 model
    # as documented in the README. If you use a different model, you may need to adjust these patterns.
...
if [ "$SKIP_VIDEO_STEPS" == "false" ]; then
    ...
    # Find the VLM output directories (same patterns as Step 4)
    COARSE_VLM_JSON=$(find "$HGTREE_OUTPUT_DIR" -path "*/LLaVA-Video-7B-Qwen2_*_coarse/maxf64_*_Here is a .json" | head -1)
    FINE_VLM_JSON=$(find "$HGTREE_OUTPUT_DIR" -path "*/LLaVA-Video-7B-Qwen2_*_fine/maxf64_*_Here is a .json" | head -1)
...
# Note: These patterns match the output structure from DeepSeek-R1-Distill-Qwen-14B model
# as documented in the README. Directory names include model identifiers.
...
# Note: The directory name includes refinement parameters (VxV10_nn10_tao0.1) as generated by refinement_eval.py
